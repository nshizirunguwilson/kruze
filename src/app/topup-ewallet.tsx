  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top + 8 }]}>
      ...
      <View style={styles.headerWrap}>
        <Header title="Top Up E-Wallet" onBack={() => router.back()} />
      </View>
      ...
      <ScrollView
        ...
        <Pressable style={styles.addCard} onPress={() => router.push('/add-card')}>
        ...
        <View style={styles.group}>
          <Method label="Paypal" icon={<Ionicons name="logo-paypal" size={22} color="#1A4F9C" />} selected={selected === 'Paypal'} onPress={() => setSelected('Paypal')} />
          <View style={styles.groupDivider} />
          <Method label="Apple Pay" icon={<AppleIcon size={22} />} selected={selected === 'Apple Pay'} onPress={() => setSelected('Apple Pay')} />
          <View style={styles.groupDivider} />
          <Method label="Google Pay" icon={<GoogleIcon size={22} />} selected={selected === 'Google Pay'} onPress={() => setSelected('Google Pay')} />
        </View>
      </ScrollView>
...
function Method({ label, icon, selected, onPress }: { label: string; icon: React.ReactNode; selected: boolean; onPress: () => void }) {
  return (
...
const styles = StyleSheet.create({
  ...
  addCard: { height: 64, borderRadius: 14, borderWidth: 1, borderColor: colors.border, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  ...
  method: { height: 64, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  ...
  bottomBar: {
    position: 'absolute', bottom: 0, left: 0, right: 0, paddingHorizontal: 24, paddingTop: 14,
    backgroundColor: colors.white, borderTopLeftRadius: 24, borderTopRightRadius: 24,
    shadowColor: '#1B2A4A', shadowOpacity: 0.08, shadowRadius: 16, shadowOffset: { width: 0, height: -4 }, elevation: 10,
  },
});
